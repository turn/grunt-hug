...
var hug = module.exports = function(options, callback){
	...
	function processSourceList(){
		...
		// we're done		
		waitingCounter = srcList.length;
		while(srcPath = srcList.shift()){
			config.debug('[Hug] Processing: ' + srcPath);
			hugModule(srcPath, config, function(srcPath, src){
...
function createConfig(options){
	...
	headList = headList.map(path.resolve.bind(path));
	...
	basePaths.map(path.resolve.bind(path));
	...
	try{
		function contentCallback(srcPath, src){
			...
			if(typeof src !== "string"){
				hasError = true;
				errorCallback("Unable to read source file: " + srcPath + ". " + src.message);
				return;
			}
			srcMap[srcPath] = src;
			returnCounter++;
